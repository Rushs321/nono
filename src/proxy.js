...
export async function processRequest(request, reply) {
    ...
    try {
        ...
        if (checkCompression(request)) {
            return applyCompression(request, reply, response.body); // Pass the stream directly to compressImg
        } else {
            return performBypass(request, reply, response.body); // Pass the stream directly to bypass
        }
    } catch (err) {
        return handleRedirect(request, reply);
    }
